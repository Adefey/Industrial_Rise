...
void IndustrialMapReduce::Shuffle() {
    ...
    for (size_t mapper_i = 0; mapper_i < num_mappers; ++mapper_i) {
        for (size_t mapper_j = 0; mapper_j < post_mapper[mapper_i].size(); ++mapper_j) {
            ...
            for (size_t reducer_i = 0; reducer_i < num_reducers; ++reducer_i) {
                if (keys[reducer_i].contains(post_mapper[mapper_i][mapper_j].first)) {
                    for (size_t reducer_j = 0; reducer_j < pred_reducer[reducer_i].size(); ++reducer_j) {
                        if (pred_reducer[reducer_i][reducer_j].first == post_mapper[mapper_i][mapper_j].first) {
                            pred_reducer[reducer_i][reducer_j].second.push_back(post_mapper[mapper_i][mapper_j].second);
                            break;
                        }
                    }
            ...
            if (!elem_in_reducers) {
                ...
                if (pred_reducer[cur_new_key].size() == 0) {
                    ...
                    while (it != pred_reducer[cur_new_key].end() && post_mapper[mapper_i][mapper_j].first.compare((*it).first) < 0) {
                        it++;
                    }
