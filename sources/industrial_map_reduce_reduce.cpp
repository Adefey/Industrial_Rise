...
void IndustrialMapReduce::Shuffle() {
    ...
    for (size_t mapper_i = 0; mapper_i < num_mappers; ++mapper_i) {
        for (auto const& elem : post_mapper[mapper_i]) {
            bool elem_in_reducers = false;
            for (size_t reducer_i = 0; reducer_i < num_reducers; ++reducer_i) {
                if (keys[reducer_i].contains(elem)) {
                    for (size_t reducer_j = 0; reducer_j < pred_reducer[reducer_i].size(); ++reducer_j) {
                        if (pred_reducer[reducer_i][reducer_j].first == elem) {
                            pred_reducer[reducer_i][reducer_j].second.push_back(post_mapper[mapper_i][elem]);
                            break;
                        }
                    }
                }
